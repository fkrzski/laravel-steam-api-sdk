...
The facade proxies the assertions and nothing else. Keep the `MockClient` that
`fake()` returns when a test needs the recorded traffic itself —
`getRecordedResponses()`, `getLastRequest()`:
